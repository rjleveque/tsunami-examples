...
#--------------------------
    ...
    def eta_gauge(current_data):
        eta = current_data.q[-1,:]
        if current_data.gaugeno < 20000:
            # for tide gauges, only show on finest level:
            level = current_data.level
            max_level = level.max()
            eta = np.where(level==max_level, eta, np.nan)
        return eta

    plotitem.plot_var = eta_gauge
